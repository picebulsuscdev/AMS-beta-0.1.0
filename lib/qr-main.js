class e {
    ...
    static async createQrEngine(a) {
        ...
        a = () =>
            import("/public/qr-scanner-worker.min.js").then((c) => c.createWorker());
        if (
            !(
    ...
    async setFocusAtPoint(x, y) {
      const videoTrack = this.$video.srcObject.getVideoTracks()[0];
      const capabilities = videoTrack.getCapabilities();
    
      if (capabilities.focusMode && capabilities.focusMode.includes('manual') && capabilities.focusPointX && capabilities.focusPointY) {
        try {
          await videoTrack.applyConstraints({
            advanced: [{ focusMode: 'manual', focusPointX: x, focusPointY: y }]
          });
          console.log(`Focus set at point: (${x}, ${y})`);
        } catch (error) {
          console.error("Failed to set focus point:", error);
        }
      } else {
        console.warn("Device does not support manual focus or focus points.");
      }
    }
}
